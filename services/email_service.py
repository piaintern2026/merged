...
from utils import today_pkt, now_pkt, display_role
...
def _smtp_settings(app) -> dict:
    ...
    value an Admin has saved on the Email Settings page
...
def send_test_email(recipient: str) -> tuple[bool, str]:
    ...
    Admin gets immediate success/failure feedback (rather than
...
def _base_url() -> str:
    ...
    Preference order:
      ...
      2. The Host header of the current request, if we're inside a
         request context (e.g. an Admin clicking "Create User" on
...
# ---------------------------------------------------------------------
def get_rotation_pm_recipient(rotation) -> str | None:
    """
    Resolve the single, correct Project Manager recipient for an
    intern-rotation notification: the PM the intern is being rotated
    *to* (`rotation.to_manager`).

    Deliberately narrow - this must NEVER fall back to "all HR",
    "all PMs", or any other broad list. Returns None (instead of
    raising or silently defaulting to a broad audience) whenever a
    single, valid, active PM email cannot be resolved, so the caller
    can skip sending gracefully.
    """
    manager = getattr(rotation, "to_manager", None)
    if not manager:
        return None
...
def send_rotation_pm_notification_email(rotation) -> None:
    """
    Notify ONLY the receiving Project Manager that an intern has been
    rotated into their department. This must never be broadened to
    HR, other PMs, admins, interns, or engineers - see
    `get_rotation_pm_recipient`.

    If no valid PM recipient can be resolved (e.g. the target manager
    has no linked/active user account), this is a no-op - it logs and
    returns instead of falling back to a broader audience.
    """
    recipient = get_rotation_pm_recipient(rotation)
    if not recipient:
        current_app.logger.warning(
            "email_service: rotation #%s has no resolvable active Project "
            "Manager recipient - skipping rotation notification email.",
            getattr(rotation, "id", "?"),
        )
        return
    ...
    send_hr_pm_notification_email(
        recipients=[recipient],  # single-element list: exactly one PM, no duplicates
        recipient_name=rotation.to_manager.full_name,
        event_title="Intern Rotation Processed",
        event_message=f"{intern.full_name} has been rotated into your department.",
        details=[
...
# ---------------------------------------------------------------------
# 9. Staff account creation (Station HR / Project Manager / Admin)
#    Interns get send_welcome_email() above; this covers every other
#    role created from the Admin "Users" / "Project Managers"
...
def send_staff_account_email(user, raw_password: str, created_by: str | None = None) -> None:
    ...
    display_role_name = display_role(user.role)

    send_email(
        subject=f"Your {display_role_name} Account Has Been Created - Intern Onboarding Portal",
        recipients=user.email,
