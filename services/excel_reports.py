...
LOGO_PATH = os.path.join(BASE_DIR, "static", "images", "iop-logo.png")

REPORT_ORG_NAME = "Intern Onboarding Portal"
REPORT_SYSTEM_NAME = "Intern Onboarding Portal"
REPORT_CONFIDENTIALITY_NOTE = "Confidential - For Internal Management Use Only"

# Brand colors (matching static/css/style.css), as ARGB hex for OpenPyXL
...
def _new_sheet(title: str, subtitle: str, header: list):
    """Create a workbook + worksheet with a branded letterhead
...
def build_intern_progress_excel(
    ...
    profile_rows = [
        ...
        ["Major", intern.major or "-"],
        ["Documents Status", intern.documents_status],
        ["Certificate Status", intern.certificate_status],
    ...
    sub_header = ["Link / File", "Submitted"]
    for col_index, col_name in enumerate(sub_header, start=1):
        ...
        cell.border = THIN_BORDER
    sub_rows = [
        [s.link or (s.original_filename or "-"), s.submitted_at.strftime("%d %b %Y")]
...
def build_project_summary_excel(projects: list) -> BytesIO:
    header = ["Title", "Department", "Division/Section", "Manager", "Intern(s)", "Status", "Start Date", "Deadline", "Project Brief"]
    wb, ws, start_row = _new_sheet(
...
# ------------------------------------------------------------------------
# 6. Rotation Report (org-wide, all rotations)
# ------------------------------------------------------------------------
def build_rotation_report_excel(rotations: list) -> BytesIO:
    header = ["Intern", "From Dept.", "To Dept.", "To Manager", "Start", "End", "Duration", "Reason"]
    wb, ws, start_row = _new_sheet("Rotation Report", f"{len(rotations)} rotation(s)", header)
...
# ------------------------------------------------------------------------
# 7. Internship Report (org-wide summary, one row per intern)
# ------------------------------------------------------------------------
def build_internship_report_excel(rows: list) -> BytesIO:
    header = ["Intern", "Department", "Manager", "Status", "Completion %", "Attendance %", "Days Remaining"]
    wb, ws, start_row = _new_sheet("Internship Report", f"{len(rows)} intern(s)", header)
    ...
    _write_rows(ws, table_rows, start_row, len(header))
    _write_footer(ws, start_row + len(table_rows), len(header))
    return _finalize(wb)
