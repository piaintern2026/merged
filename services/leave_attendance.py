...
def sync_attendance_for_leave(leave) -> None:
    ...
    Runs regardless of whether the intern currently has a
    rotation-assigned Project Manager -- the Attendance table has no
    PM column and never needed one; requiring one here only meant
    leaves for interns who had never been through Rotation Management
    silently produced no attendance records at all when approved.
    """
