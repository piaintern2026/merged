...
config.py
---------
Central configuration for the Intern Onboarding Portal.
...
class Config:
    ...
    # Local-dev-only fallback folder for intern portal submissions
    # (Project Submission attachments + Final Internship Report).
    # Organised into subfolders per type so files never collide across
    # the different upload categories. In production this is bypassed
    # entirely in favour of Vercel Blob (see BLOB_READ_WRITE_TOKEN
    # below and services/file_storage.py) -- Vercel's serverless
    # functions don't have a writable/persistent local filesystem, so
    # this folder is never actually used once that token is set.
    ...
    # Read/write token for Vercel Blob, the persistent object storage
    # used for every file an intern uploads (Project Submission
    # attachments, Final Internship Report attachments). Automatically
    # populated by Vercel when a Blob store is connected to the
    # project; set it manually in a local .env only if you want to test
    # against Blob storage instead of the local-disk fallback above.
    ...
    # ------------------------------------------------------------------
    # TEMPORARY DEMO SWITCH -- hides Station HR from navigation, the
    # dashboard, and the Users & Roles list, and blocks the two
    # HR-only admin routes (Add Station HR, Station-HR-filtered user
    # list) for everyone, including Admin.
    #
    # This does NOT touch the database, models, HR-owned data, the
    # "Station HR" role itself, or any route's permission checks
    # (existing Station HR user accounts can still log in and use the
    # app normally; roles_required("Station HR") is unchanged) -- it
    # only hides HR-related UI and closes the two admin entry points
    # listed above for the duration of the demo.
    #
    # To re-enable HR: set this back to False (or delete the line) and
    # restart the app. No other change is required.
    # ------------------------------------------------------------------
    HIDE_HR_MODULE = os.environ.get("HIDE_HR_MODULE", "true").lower() in ("1", "true", "yes")

    # Default HR account, auto-created on first run.
    DEFAULT_HR_EMAIL = "hr@piac.com"
    DEFAULT_HR_PASSWORD = "piacl@2026"
    DEFAULT_HR_ROLE = "Station HR"

    # Default Admin account, auto-created on first run.
    DEFAULT_ADMIN_USERNAME = "piaadmin"
    DEFAULT_ADMIN_EMAIL = "piaadmin@piac.com"
    DEFAULT_ADMIN_PASSWORD = "piacl@2026"
    ...
    MAIL_DEFAULT_SENDER = (
        os.environ.get("MAIL_DEFAULT_SENDER_NAME", "Intern Onboarding Portal"),
