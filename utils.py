...
# user account (Admin/Station HR via routes/admin.py, or the
...
def roles_required(*roles):
    ...
    "Admin" always passes this check regardless of which roles
    ...
    implicitly reachable by an Admin without needing to list
    "Admin" at every call site.
    """
...
def display_role(role: str | None) -> str:
    ...
    The stored role values already match what should be shown on
    screen ("Admin", "Station HR", "Project Manager", "Intern"), so
    this is currently an identity mapping. Kept as a single seam
    (rather than removing the filter and every `|display_role` call
    site) so that if a role's on-screen label ever needs to diverge
    from its stored/authorization value again, there's exactly one
    place to change it -- callers that compare against
    `current_user.role` or filter/query by role must keep using the
    real stored value regardless.
    """
    return role


def hr_module_visible() -> bool:
    """
    Temporary demo switch (Config.HIDE_HR_MODULE). Returns False while
    HR is hidden for a demo, True otherwise. Used to hide HR-related
    nav items/buttons/cards in templates and to close the two
    HR-only admin routes for the duration of the demo. Does not
    affect the database, models, or any existing permission check --
    see config.py for how to re-enable.
    """
    return not current_app.config.get("HIDE_HR_MODULE", False)


def current_hr_city():
    """
    Return the city the currently logged-in Station HR user is scoped
    to, or None if the current user isn't a Station HR (Admin and
...
def hr_city_scope():
    """
    Return (is_city_scoped, city) for the current user. Admin is
