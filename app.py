...
def create_app(config_class: type = Config) -> Flask:
    ...
    # Temporary demo switch -- see Config.HIDE_HR_MODULE in config.py.
    # {% if hr_module_visible() %} ... {% endif %} in templates.
    app.jinja_env.globals["hr_module_visible"] = hr_module_visible
    ...
    with app.app_context():
        ...
        seed_default_admin_account(app)
...
def run_schema_migrations() -> None:
    ...
    # Admin User Management: account-security columns added after
    ...
    # Role rename: "Super Admin" -> "Admin" (display-name-only change
    # promoted to the actual stored/authorization role value). `role`
    # is a plain VARCHAR column (models/user.py), not a DB-level enum,
    # so no schema change is needed here -- only the *data* in any
    # existing row still holding the old label needs to be updated so
    # it matches what every route/template/decorator now checks for
    # (`roles_required("Admin")`, `current_user.role == "Admin"`,
    # `config.DEFAULT_ADMIN_ROLE`, etc). Idempotent and safe to run on
    # every startup: a no-op once every row already says "Admin", and
    # it only ever UPDATEs the role label in place -- no rows are
    # added, removed, or otherwise touched, so no account or its
    # permissions/history are lost.
    if "users" in inspector.get_table_names():
        from flask import current_app

        result = db.session.execute(
            text("UPDATE users SET role = 'Admin' WHERE role = 'Super Admin'")
        )
        db.session.commit()
        if result.rowcount:
            current_app.logger.info(
                "role migration: updated %s user(s) from 'Super Admin' to 'Admin'.",
                result.rowcount,
            )
...
def seed_default_admin_account(app: Flask) -> None:
    """
    Ensure the default Admin account exists. Runs once at startup
    ...
    existing = User.query.filter(
        ...
        | (User.email == app.config["DEFAULT_ADMIN_EMAIL"])
    ).first()
    if existing:
        return
    ...
    default_admin = User(
        ...
        role=app.config["DEFAULT_ADMIN_ROLE"],
        full_name="Admin",
    )
    ...
    db.session.commit()
    app.logger.info(
        "Default Admin account created: %s", app.config["DEFAULT_ADMIN_USERNAME"]
    )
