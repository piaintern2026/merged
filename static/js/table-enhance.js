 ...
 * table-enhance.js
 * ----------------
 * Generic, reusable enhancer for data tables. For every <table
...
(function () {
    ...
    /**
     * escapeDropdownClipping()
     * ------------------------
     * Fix for HR pages: any Bootstrap dropdown toggled from *inside* a
     * scrollable ancestor (e.g. the sticky-header .pia-table-wrap, which
     * needs `overflow: auto` for its sticky <thead>) gets visually
     * clipped or hidden behind the table/sticky header, no matter how
     * high its z-index is set -- overflow:auto on an ancestor clips
     * position:absolute descendants regardless of stacking order.
     *
     * The fix: when such a dropdown opens, temporarily switch it to
     * `position: fixed`, compute its on-screen coordinates from the
     * toggle button's bounding rect, and restore it to normal flow when
     * closed. This keeps Bootstrap's own dropdown logic (open/close,
     * keyboard nav) completely intact -- only the positioning strategy
     * changes for menus that would otherwise be clipped.
     */
    function escapeDropdownClipping() {
        document.addEventListener("show.bs.dropdown", function (event) {
            ...
            const clippingAncestor = toggle.closest(".pia-table-wrap, .modal-body");
            if (!clippingAncestor) return; // no clipping risk, leave Bootstrap's default positioning alone
            ...
            requestAnimationFrame(function () {
                ...
                menu.classList.add("pia-dropdown-escaped");
                menu.style.top = rect.bottom + 4 + "px";

                // Keep the menu on-screen horizontally (align right edge
                // to the toggle if it would otherwise overflow the
                // viewport, matching Bootstrap's own auto-flip behaviour).
                let left = rect.left;
        ...
        document.addEventListener("hidden.bs.dropdown", function (event) {
            ...
            const menu = toggle.nextElementSibling && toggle.nextElementSibling.classList.contains("dropdown-menu")
                ...
                : toggle.parentElement.querySelector(".dropdown-menu");
            if (!menu) return;
            menu.classList.remove("pia-dropdown-escaped");
            menu.style.top = "";
            menu.style.left = "";
        });
    }

    document.addEventListener("DOMContentLoaded", escapeDropdownClipping);
})();
