...
- `templates/admin/email_settings.html` - Super Admin SMTP config + test email + log viewer.
...
- `templates/components/sidebar.html` - "Email Settings" (Super Admin) and "Announcements"
  (Super Admin + Station HR) nav links.
...
3. A Super Admin can then edit these live from **Email Settings** without redeploying; the
