...
class User(UserMixin, db.Model):
    ...
    # Role-based access control. One of: 'Super Admin', 'Station HR',
    ...
    # (i.e. Station HR -- Project Manager/Intern already store full_name on
    # their own profile rows). Nullable so existing rows stay valid.
    full_name = db.Column(db.String(120), nullable=True)

    # City this account is scoped to. Mandatory for 'Station HR' accounts
    # only (a Station HR manages/views interns, projects, attendance,
    # reports and analytics for exactly one city). Nullable for every
    # other role (Super Admin isn't scoped to any single city; Project
    # Manager/Intern already have their own city/station fields on their
    # profile tables).
    ...
    # Account security / admin-management fields.
    ...
    # A locked account must also be blocked from logging in.
    @property
