...
class InternRotation(db.Model):
    ...
    # Sub Department at each end of the rotation, mirroring
    # Intern.sub_department_id -- nullable because not every department
    # has sub-departments and older rotation rows predate this field.
    from_sub_department_id = db.Column(db.Integer, db.ForeignKey("sub_departments.id"), nullable=True)
    to_sub_department_id = db.Column(db.Integer, db.ForeignKey("sub_departments.id"), nullable=True)
    ...
    from_sub_department = db.relationship("SubDepartment", foreign_keys=[from_sub_department_id])
    to_sub_department = db.relationship("SubDepartment", foreign_keys=[to_sub_department_id])
