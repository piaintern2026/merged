...
def send_reminders():
    """On-demand version of the 'flask send-hr-reminders' CLI command:
    generates reminders for overdue projects, pending evaluations and
    interns due for a rotation review right now, without waiting for
    the scheduled job. Duplicate reminders for the same day are
    ...
    try:
        ...
        if total:
            flash(
                f"Sent {total} reminder(s): {counts['overdue_projects']} overdue project, "
                f"{counts['pending_evaluations']} pending evaluation, "
                f"{counts['upcoming_rotations']} upcoming rotation.",
                "success",
            )
        else:
            flash("No new reminders to send -- everything is already up to date.", "info")
    except Exception:
        db.session.rollback()
        current_app.logger.exception("Failed to generate HR reminders.")
        flash("Could not generate reminders due to a system error. Please try again.", "danger")
    return redirect(request.referrer or url_for("notification.list_notifications"))
