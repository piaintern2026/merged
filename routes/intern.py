...
def add_intern():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # In-app notifications for HR/Super Admin so a new registration
            # doesn't go unnoticed even if email delivery fails.
            for staff in User.query.filter(
                User.role.in_(["Station HR", "Super Admin"]), User.is_active_account.is_(True)
            ).all():
                notify_user(
                    staff.id,
...
# ======================================================================
# Bulk Import Interns (Super Admin only) -- Excel (.xlsx) / CSV upload
# ======================================================================
...
def import_interns():
    ...
    if success_count:
        log_action(
            action="IMPORT",
            description=f"Super Admin imported {success_count} intern(s) via Excel.",
            target_type="Intern",
        )
