...
def statistics():
    ...
    chart data (departments, cities, attendance, leave, projects,
    rotations, monthly interns) for the Analytics dashboard."""
    ...
    # ------------------------------------------------------------------
    # Chart datasets - all computed from live DB rows, no dummy/static
    # numbers anywhere below.
    # ------------------------------------------------------------------

    # Interns per department (bar chart)
    dept_rows = (
    ...
    departments_chart = {
        "labels": [name for name, _ in dept_rows],
        "data": [count for _, count in dept_rows],
    }

    # Interns per city (pie chart) - city is free-text on Intern, so
    # group in Python to normalise case/whitespace instead of relying
    # on the DB collation.
    city_counts = {}
    ...
    # Attendance breakdown by status (doughnut chart)
    ...
    attendance_chart = {
        "labels": list(attendance_status_counts.keys()),
        "data": list(attendance_status_counts.values()),
    }
    ...
    leave_chart = {
        "labels": [status for status, _ in leave_rows],
        "data": [count for _, count in leave_rows],
    }

    # Projects by status (reuse project_status_counts, already computed)
    projects_chart = {
        "labels": list(project_status_counts.keys()),
        "data": list(project_status_counts.values()),
    }
    ...
    rotations_chart = {
        "labels": [name for name, _ in rotation_rows],
        "data": [count for _, count in rotation_rows],
    }

    # New interns onboarded per month, last 12 months (line chart)
    today = date.today()
    month_labels = []
    month_keys = []
    ...
    monthly_interns_chart = {
        "labels": month_labels,
        "data": [monthly_counts[key] for key in month_keys],
    }
...
MANAGEABLE_ROLES = ("Super Admin", "Station HR")

# Super Admin accounts can no longer be created from the UI -- only via
# direct database/manual creation. "Station HR" is the only role selectable when
# adding a brand-new account. Existing Super Admin accounts are untouched
# and can still be edited (see edit_user, which allows keeping the
# "Super Admin" role for an account that already has it).
...
def list_users():
    ...
    ?role=HR (used by the "HR Management" sidebar link so Super Admin
    can jump straight to HR accounts specifically)."""
    role_filter = request.args.get("role", "")
    status_filter = request.args.get("status", "")
    search_text = request.args.get("q", "").strip()

    query = User.query.filter(User.role != "Super Admin")
    if role_filter:
    ...
    all_roles = [
        row[0]
        for row in db.session.query(User.role).filter(User.role != "Super Admin").distinct().all()
    ]
...
def add_user():
    ...
    if request.method == "POST":
        ...
        if role not in CREATABLE_ROLES:
            errors.append("Please select a valid role. Super Admin accounts cannot be created from the UI.")
...
def edit_user(user_id):
    ...
    # Super Admin can no longer be assigned from the UI. If this account
    # is already a Super Admin, its role dropdown still offers "Super
    ...
    # Super Admin through this form.
    editable_roles = MANAGEABLE_ROLES if user.role == "Super Admin" else CREATABLE_ROLES
...
def reset_user_password(user_id):
    """Reset any user's password to a value the Super Admin sets, and
...
# ----------------------------------------------------------------------
# 7. Internship Progress Tracking (Super Admin only)
# ----------------------------------------------------------------------
...
def internship_progress():
    ...
    Super Admin sees every city with no restriction."""
...
# ----------------------------------------------------------------------
# 8. Workforce Insights (Super Admin / HR) -- merges the former
...
def workforce_insights():
    ...
    interns, etc.) in a single view. Super Admin sees every city with no
    ...
    # keyed by city instead of department. Super Admin gets every city
...
# ----------------------------------------------------------------------
# 10. Database Backup (Super Admin only) -- part of the Quick Actions panel
# ----------------------------------------------------------------------
...
def backup_database():
    ...
    production) aren't file-backed, so we point the Super Admin to the
...
# ----------------------------------------------------------------------
# Email Notification System: Super Admin Email Settings
# ----------------------------------------------------------------------
...
@login_required
@roles_required("Super Admin", "Station HR")
def email_settings():
    """Configure SMTP settings, send a test email, and review the
    email delivery log -- all from one Super Admin-only page."""
    if request.method == "POST":
        updated = []
...
# ----------------------------------------------------------------------
# Announcements: broadcast a message (with email + in-app notification)
# to all Interns, all Project Managers, all Station HR, or everyone.
# ----------------------------------------------------------------------
@admin_bp.route("/announcements", methods=["GET", "POST"])
@login_required
@roles_required("Super Admin", "Station HR")
def announcements():
    """Post a new announcement and view announcement history."""
    if request.method == "POST":
        ...
        if audience not in Announcement.AUDIENCES:
            errors.append("Please select a valid audience.")
        ...
        try:
            if audience == "All":
                recipients_users = User.query.filter_by(is_active_account=True).all()
            else:
                recipients_users = User.query.filter_by(role=audience, is_active_account=True).all()
            ...
            announcement = Announcement(
                ...
                recipient_count=len(recipients_users),
            )
            ...
            log_action(
                action="CREATE",
                description=f"Posted announcement '{title}' to audience: {audience} ({len(recipients_users)} recipients).",
                target_type="Announcement",
            )
            db.session.commit()

            recipient_emails = [u.email for u in recipients_users if u.email]
            if recipient_emails:
                send_announcement_email(
                    recipient_emails, title, body, posted_by=current_user.display_name()
                )

            flash(f"Announcement posted to {len(recipients_users)} recipient(s).", "success")
        except Exception:
            db.session.rollback()
            current_app.logger.exception("Failed to post announcement.")
            flash("Could not post the announcement due to a system error. Please try again.", "danger")

        return redirect(url_for("admin.announcements"))

    history = Announcement.query.order_by(Announcement.created_at.desc()).limit(50).all()
    return render_template(
        "admin/announcements.html", history=history, audiences=Announcement.AUDIENCES
    )
