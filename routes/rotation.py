...
from models import Intern, Department, ProjectManager, Project, InternRotation, Attendance, Evaluation, Leave
from utils import roles_required, log_action, notify_user, hr_city_scope
from services.email_service import send_rotation_email, send_rotation_pm_notification_email
...
def list_rotations():
    ...
    if is_city_scoped:
        query = query.join(Intern, InternRotation.intern_id == Intern.id).filter(Intern.station == hr_city)
    if department_id:
        query = query.filter(
            db.or_(
    ...
    if intern_id:
        query = query.filter(InternRotation.intern_id == intern_id)
...
def rotate_intern(intern_id):
    ...
    if is_city_scoped and intern.station != hr_city:
        ...
        return redirect(url_for("intern.list_interns"))
    departments = Department.query.order_by(db.func.lower(Department.name)).all()
    managers = ProjectManager.query.filter_by(is_active_flag=True).order_by(db.func.lower(ProjectManager.full_name)).all()
    projects = Project.query.order_by(db.func.lower(Project.title)).all()
    current_manager = _current_manager(intern)

    if request.method == "POST":
        to_department_id = request.form.get("to_department_id")
        to_sub_department_id = request.form.get("to_sub_department_id") or None
        ...
        to_sub_department = None
        if to_sub_department_id and not errors:
            from models import SubDepartment

            to_sub_department = SubDepartment.query.get(int(to_sub_department_id))
            if not to_sub_department or (
                to_department_id and to_sub_department.department_id != int(to_department_id)
            ):
                errors.append("The selected sub department does not belong to the target department.")
                to_sub_department = None
        ...
        try:
            ...
            previous_sub_department_id = intern.sub_department_id
            ...
            rotation = InternRotation(
                ...
                from_sub_department_id=previous_sub_department_id,
                to_sub_department_id=to_sub_department.id if to_sub_department else None,
            ...
            # Keep the intern's live "current department"/"current sub
            # department" columns in sync -- a rotation to a new
            # department without an explicit target Division/Section
            # clears the stale sub-department instead of leaving it
            # pointing at a sub-department under the *old* department.
            intern.department_id = int(to_department_id)
            intern.sub_department_id = to_sub_department.id if to_sub_department else None
            ...
            if rotation.to_manager and rotation.to_manager.user_id:
                notify_user(
                    user_id=rotation.to_manager.user_id,
                    message=(
                        f"{intern.full_name} has been rotated into your department "
                        f"({rotation.to_department.name})."
                    ),
                    icon="bi-arrow-left-right",
                    notification_type="General",
                )
            if previous_manager and previous_manager.user_id and (
                not rotation.to_manager or previous_manager.id != rotation.to_manager.id
            ):
                notify_user(
                    user_id=previous_manager.user_id,
                    message=(
                        f"{intern.full_name} has been rotated out of your department "
            ...
            # Rotation notification emails must reach ONLY the receiving
            # Project Manager - never HR, other PMs, admins, interns, or
            # engineers. `send_rotation_email` already targets solely the
            # intern; `send_rotation_pm_notification_email` resolves and
            # verifies exactly one recipient (the new PM) and is a no-op
            # (logs + skips) if no valid PM can be resolved, rather than
            # ever falling back to a broad audience.
            send_rotation_email(rotation)
            send_rotation_pm_notification_email(rotation)
...
@rotation_bp.route("/calendar")
@login_required
@roles_required("Station HR", "Admin")
def calendar():
    """HR Calendar: a single view combining recent/upcoming rotations,
    leave requests, project deadlines and evaluations so HR can see
    everything scheduled at a glance. Events are gathered from a 30-day
    window centered on today (14 days back, 45 days forward) -- wide
    enough to be useful without loading the entire history."""
    today = today_pkt()
    ...
    # Rotations: recorded rotation start dates within the window.
    rotations = InternRotation.query.filter(
        InternRotation.start_date >= window_start, InternRotation.start_date <= window_end
    )
    if is_city_scoped:
        rotations = rotations.join(Intern, InternRotation.intern_id == Intern.id).filter(Intern.station == hr_city)
        ...
        events.append(
            {
                "date": r.start_date,
                "type": "Rotation",
                "title": f"{r.intern.full_name} \u2192 {r.to_department.name}",
                "url": url_for("rotation.timeline", intern_id=r.intern_id),
            }
        )

    # Leave: every leave day within the window, for Pending/Approved requests.
    ...
    if is_city_scoped:
        leaves = leaves.join(Intern, Leave.intern_id == Intern.id).filter(Intern.station == hr_city)
    leaves = leaves.all()
    for lv in leaves:
        events.append(
            {
                "date": lv.start_date,
                "type": "Leave",
                "title": f"{lv.intern.full_name} \u2014 {lv.leave_type} ({lv.status})",
                "url": url_for("leave.leave_records"),
            }
        )

    # Project deadlines within the window (still open projects).
    projects = Project.query.filter(
        Project.deadline >= window_start,
        Project.deadline <= window_end,
        Project.status.notin_(["Completed", "Approved"]),
    )
    if is_city_scoped:
        projects = projects.join(Department, Project.department_id == Department.id).filter(
            Department.city == hr_city
        )
    ...
    # Evaluations recorded within the window (HR Final + PM).
    evaluations = Evaluation.query.filter(
        Evaluation.created_at >= datetime.combine(window_start, datetime.min.time()),
        Evaluation.created_at <= datetime.combine(window_end, datetime.max.time()),
    )
    if is_city_scoped:
        evaluations = evaluations.join(Intern, Evaluation.intern_id == Intern.id).filter(Intern.station == hr_city)
        ...
        events.append(
            {
                ...
                "type": "Evaluation",
                "title": f"{e.intern.full_name} \u2014 {e.evaluation_type} evaluation",
                "url": url_for("evaluation.view_evaluation", evaluation_id=e.id),
            }
        )

    # Internship end dates within the window (upcoming completions).
    ending_interns_query = Intern.query.filter(
        Intern.internship_end_date >= window_start, Intern.internship_end_date <= window_end
    )
    if is_city_scoped:
        ending_interns_query = ending_interns_query.filter(Intern.station == hr_city)
    ending_interns = ending_interns_query.all()
    for i in ending_interns:
        events.append(
            {
                "date": i.internship_end_date,
                "type": "Completion",
                "title": f"{i.full_name} internship ends",
                "url": url_for("intern.view_intern", intern_id=i.id),
            }
        )
    ...
    for e in events:
        events_by_date[e["date"].isoformat()].append(e)

    upcoming = [e for e in events if e["date"] >= today][:25]

    return render_template(
        "rotations/calendar.html",
        events_json=[{**e, "date": e["date"].isoformat()} for e in events],
        upcoming=upcoming,
        today=today,
    )
...
def dashboard():
    ...
    is_city_scoped, hr_city = hr_city_scope()
    departments_query = Department.query.order_by(db.func.lower(Department.name))
    interns_query = Intern.query
    rotations_query = InternRotation.query
    if is_city_scoped:
        departments_query = departments_query.filter(Department.city == hr_city)
        interns_query = interns_query.filter(Intern.station == hr_city)
        rotations_query = rotations_query.join(Intern, InternRotation.intern_id == Intern.id).filter(
            Intern.station == hr_city
        )
    departments = departments_query.all()
    all_rotations = rotations_query.all()

    dept_time = defaultdict(int)
    dept_rotation_count = defaultdict(int)
    dept_project_ids = defaultdict(set)

    for intern in interns_query.all():
